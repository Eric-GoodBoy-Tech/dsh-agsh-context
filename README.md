...
第 3 步的内层 `ctx.llm.stream(innerOptions)` **实时放行**:每个 chunk 收到即交给 DSH,思维链 / 正文 / 工具调用都以模型生成速度到达 UI。判定与重试分三层:

- **漂移就地改写** — shell tool-call 的参数名漂移(如把 `cmd` 写成 `command`、`cmd` 为空串)时,就地改写 `block-end` 的 `arguments` 为规格键。不重发、不中断;前端与节点 history 拿到的都是规范参数。
- **内容级重试(step 级)** — 参数不可用(非法 JSON / 无有效值 / 装配失败)且非 `max-tokens` 截断时,本次尝试以 error 收尾(失败码 `AGSH_TOOLCALL`);插件在 `agent/request-error` 上认领该码,写入 `llm/retry` + `llm/retry-started` 后请求 DSH **重跑本 step**。DSH 开一次新的 attempt,前端据此擦掉失败尝试已流出的内容再重新渲染。失败尝试不写节点 history。
- **流以 `error` 收尾** — 原样放行,交给 DSH 原生 `dsh-llm-retry`(按 provider 的 retryPolicy,默认 5 次)。

**预算**:环境变量 `AGSH_API_RETRY_MAX`(含首次,默认 2 ⇒ 每个用户回合最多自动重发 1 次;非法值回落 2),按会话记账、新用户消息清零。预算耗尽后不再合成 error:坏参数原样走到工具边界(工具报错 → 模型下一步自纠),回合不会因为「重试用完」而失败。

**为何不流内重发**:流内「丢弃整次尝试再重发」必须先把整次调用的全部 chunk 攥在手里才能决策——代价是思维链被压到流末尾一次性倾泻(实测每块间隔 0.03ms、整段 28ms 内到齐;同 provider/模型下标准预设是 78ms/块、铺开 10 秒)。改走 step 级重试后,实时显示与重试不再互相牺牲。

**为何幂等**:失败尝试一律不写节点 history,命令执行发生在流成功结束之后 ⇒ 重发时命令未执行、history 未写。

**与标准库的关系**:分类规格镜像 `agsh-stdlib/api-retry-core.ts`(仅指针,无跨仓 import);该 core 供标准模式的 `api-retry.ts` 适配器使用(包装 `agsh.api.stream`),与本插件目标不同、各自独立。

**借用 DSH 事件的边界**:`llm/retry` / `llm/retry-started` 的字段逐条对齐 `@deepseek-ai/dsh-llm-retry` 的持久化 invariant(turn/step 必须在开启中的 step、provider 必须等于该 step 的路由、retry 序号按 provider + policyKey 递增、failure.message/code 非空)。记录被宿主拒收时降级为放行错误,不死循环。
...
- AgentShell 运行件(`agent.zsh` + `src/cli.ts`)通过 `AGSH_ROOT` 定位(或工作目录下的 `agent-shell/src/cli.ts`)——从未装过先按 [INSTALL.md](./INSTALL.md) 第 0-2 节从零安装(写 `~/.zshrc` 与重启 DSH 两步需用户确认);AgentShell 框架独立发布,不随本包分发
- 机器前置:bun、jq、tmux
